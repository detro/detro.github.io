...
aliases:
  - /2010/01/23/prime-numbers-generator
  - /2010/01/prime-numbers-generator
---

I believe I don't have to describe what [primes](http://en.wikipedia.org/wiki/Prime_number) are, what are their properties and what not. This post is more a tribute to geek-ness of 2 friends-and-colleagues ([@lucabox](http://twitter.com/lucabox)) that have fun thinking of algorithms to solve stupid (or less stupid), and always useless problems ;-) .



![Optimus Prime](http://scottthong.files.wordpress.com/2006/08/optimus_prime.jpg)
...
* Say <code>P[i]</code> are the previously calculated Primes; If trying dividing value <code>V</code> by every <code>P[i]</code> we find that <code>P[i] &gt; sqrt(V)</code>, stop dividing and classify <code>V</code> as a newly found prime

* <strong>No need to check any even number</strong>: they are divisible by 2, so no primes by definition

* <strong>No need to allocate more space then an array of the size of the requested prime <em>ordinality</em></strong>: everything can be done in place
