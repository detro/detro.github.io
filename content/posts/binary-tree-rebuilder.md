...
aliases:
  - /2010/02/05/binary-tree-rebuilder
  - /2010/02/binary-tree-rebuilder
---

Imagine you have a [Binary Tree](http://en.wikipedia.org/wiki/Binary_tree), with those characteristics:
...
![A nice Binary Tree](http://www.drunkmenworkhere.org/bintree/pagerank.png)
...
Fortunately though, you previously did a [Pre-Order and an In-Order visit of your tree](http://en.wikipedia.org/wiki/Tree_traversal), and stored the result in an 2 nice array.
...
* Find the position of this New Node in the InOrder Array, scanning it from <code>0</code> to <code>n-1</code> (<code>n</code> is the number of Nodes)

* IF next element in the PreOrder Array is on the left of the New Node in the InOrder array: call RECURSIVELY this procedure, this time taking into account the portion of InOrder array that goes from <code>0</code> to the <code>position of the New Node in the InOrder Array -1</code>.

* IF next element in the PreOrder Array is on the right of the New Node in the InOrder array: call RECURSIVELY this procedure, this time taking into account the portion of InOrder array that goes from the<code> position of the New Node in the InOrder Array +1</code> to <code>n-1</code>.
...
This time it's not pure [C](http://en.wikipedia.org/wiki/C_(programming_language)): I wanted to use the beautiful [Qt Framework](http://qt.nokia.com/) that I'm using since July/August 2009 for work. I will attach the code full code into a ZIP file; here I report at least the core algorithm:
...
Mmm, I can see that the code doesn't look very clear in here. Well, again, the code is downloadable from: [TreeRebuilder.tar.gz](http://www.detronizator.org/wp-content/uploads/2010/02/TreeRebuilder.tar.gz)

For Qt experts: yes, I'm using just the build-mechanism of Qt and the containers. All the rest is just plain [C++](http://en.wikipedia.org/wiki/C%2B%2B). But using [Signals and Slots](http://doc.trolltech.com/4.6/signalsandslots.html) for this would have been a massive overkill, don't you think? ;)
