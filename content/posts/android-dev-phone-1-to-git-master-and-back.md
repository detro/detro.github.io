...
date: "2009-03-08T19:15:19Z"
aliases:
  - /2009/03/08/android-dev-phone-1-to-git-master-and-back
  - /2009/03/android-dev-phone-1-to-git-master-and-back
---

![Android Dev Phone 1](http://3.bp.blogspot.com/_X224WPCBE5o/STsl8k5fpKI/AAAAAAAAAis/qYHiU6r9LQI/s400/android-dev-phone-1.jpg)If you, like me, have a [Android Dev Phone 1](http://developer.android.com/guide/developing/device.html#dev-phone-1) ("ADP1" from now on), you are probably wondering how to take the latest version of the [Android Source Code](http://source.android.com/), the ["GIT Master"](http://android.git.kernel.org/), compile and put on your device.
...
* [Download](http://d.android.com/sdk/1.1_r1/index.html) and [Install](http://d.android.com/sdk/1.1_r1/installing.html) the Android SDK

* [Configure the Android SDK for "On Hardware Debuging"](http://d.android.com/guide/developing/device.html#setting-up)

* [Get the Source of the "GIT Master" from the Official Repository, get ready for Compilation <strong>BUT DO NOT COMPILE YET</strong>](http://source.android.com/download); this means stop when the how to says to execute the <code>make</code> command

* Download and Unzip [JesusFreke](http://jf.andblogs.net/) (nice name, isn't it ;-) ) for ADP1; sometimes the ADP1 is also known as "Holiday" device

A couple of notes about the last task:

* JesusFreke is an great hacker that maintain an interesting firmware for both T-Mobile G1 and ADP1

* the ADP1 is also known as "Holiday" because is the same device that the Google Employees received during some holiday period from their company as gift; and from there the first unlocked firmware were ripped ;)
...
The Android uses as default build profile the SDK: means that if you do a build now (remember? the <code>make</code> command), it will build the system image for the [QEMU Emulator](http://www.nongnu.org/qemu/) (the one the Android Emulator is based upon).

So, to build for our phone, we need to play a little bit with the build profile. To do so just follow the official how to page about "[Building for Dream](http://source.android.com/documentation/building-for-dream)". Of course, "[Dream](http://en.wikipedia.org/wiki/HTC_Dream)" is the code name of the device used by [HTC](http://en.wikipedia.org/wiki/HTC_Corporation).
...
![Android in fastboot mode](http://farm4.static.flickr.com/3017/3249806989_43c89e7571.jpg)
...
![The menu of JesusFreke recovery.img](http://farm4.static.flickr.com/3258/3156192431_930b92e59e.jpg)
...
* Unzip the JesusFreke you previously downloaded and bear in mind where you put it (for example [code lang="bash"]/home/me/jf/[/code]

* Put your ADP1 in "fastboot mode" as shown in the previous section

* Now, it's time to boot the phone from the recovery image of JF
[code lang="bash"]
$ cd /home/me/jf/data
$ sudo /home/me/mydroid/out/host/[your computer platform]/bin/fastboot boot recovery.img
[/code]

* Wait for the recovery.img to finish booting and <strong>use the menu on screen to create a backup</strong>; it will be created on the SD card under [code lang="bash"]/nandroid/[/code]

* Once done, use the right option on the menu screen to reboot the phone, mount the SD card on your computer and save the backup somewhere safe
...
[He/She maintains a blog](http://jf.andblogs.net/) with regular releases of his Android built, based on the stable releases of Google, but enhanced with some "spicy and yummy" stuff. While I'm writing this the latest build for ADP1 is [JFv1.43RC9](http://jf.andblogs.net/2009/03/03/jfv143-rc9/).
...
* Download the latest JesusFreke

* Rename it as "update.zip"

* Put on the root directory of the SD Card of your ADP1

* Turn off the Phone

* Turn it on <strong>while holding the home button</strong>

* Open the keyboard if closed

* <strong>Type ALT+L to enable the "logging"</strong>

* <strong>Type ALT+W to wipe your data from the phone (sorry, it's necessary at the first upgrade)</strong>

* <strong>Type ALT+S to apply the update.zip</strong>

* When done, click HOME+BACK to reboot
...
* [Taking Android "cupcake" out for a spi](http://blog.makezine.com/archive/2009/01/taking_android_cupcake_for_a_spin.html)n

* [Backup/install/restore Android on an Android Developer Phone 1 (ADP1)](http://antonmelser.org/open-source/backup-install-restore-adp1.html)

* [JesusFreke's AndBlog](http://jf.andblogs.net/)
