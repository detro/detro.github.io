...
aliases:
  - /2010/01/06/the-polite-merge-sort
  - /2010/01/the-polite-merge-sort
---

Another old-classic problem: the [Merge Sort](http://en.wikipedia.org/wiki/Merge_sort).



![Merge Sort Algorithm](http://www.personal.kent.edu/~rmuhamma/Algorithms/MyAlgorithms/Sorting/Gifs/mergeSort.gif)
...
Unfortunately in practice this is not always a good idea: <code>malloc(...)</code> and <code>free(...)</code> are [System Calls](http://en.wikipedia.org/wiki/System_call), and those are expensive to do. They <em>normally</em> require [Context Switches](http://en.wikipedia.org/wiki/Context_switch) and other system operations that I can't advice to do without properly thinking through your code.

## A more <em>"Polite"</em> Merge Sort
So, because we know the amount of memory that is going to be needed by this algorithm, why don't we design one that takes it in the beginning and doesn't bother the [Operating System](http://en.wikipedia.org/wiki/Operating_system) anymore? Good idea!
